...
const BottomNav: React.FC<BottomNavProps> = ({ currentPath, onOpenDonateSelector }) => {
    ...
    const isTrust = currentPath.startsWith("/trust");
            ...
            <div className="relative h-[88px] bg-white/95 dark:bg-gray-950/95 backdrop-blur-2xl border-t border-slate-100 dark:border-slate-800 shadow-[0_-10px_40px_rgba(0,0,0,0.05)] pointer-events-auto">
                <div className="relative h-full flex items-center px-2">
                    ...
                    <div className="flex-1 flex justify-around items-center">
                        ...
                        <Link
                            href="/trust"
                            className={`flex flex-col items-center gap-1 transition-all duration-300 ${isTrust ? 'text-primary' : 'text-slate-400 active:scale-90'}`}
                        >
                            <span className={`material-symbols-outlined text-[24px] ${isTrust ? 'fill-1 font-bold' : ''}`}>verified_user</span>
                            <span className="text-[9px] font-black tracking-wider uppercase">Trust</span>
                        </Link>
                    </div>
                </div>

                {/* Refined Circular Donate FAB */}
                <button 
                ...
                >
                    ...
                    <span className="text-[9px] font-black tracking-wider uppercase mt-2 text-slate-400">Donate</span>
                </button>
            </div>
